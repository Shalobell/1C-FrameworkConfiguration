...
Процедура Пересчитать(Команда) 
	...
	Для Каждого Строка Из Объект.СписокНоменклатуры Цикл 
		Строка.Цена = (Строка.Цена / 100)	* Процент;
		Строка.Сумма = (Строка.Сумма / 100)	* Процент;
	КонецЦикла; 
...
Процедура ВидЦеныПриИзменении(Элемент)	
	...
	//30.03.24 Обработка курса валют
	Объект.НаименованиеВалюты = ПредопределенноеЗначение("Справочник.Валюты.RUB"); 
	Объект.ПредыдущееНаименованиеВалюты = Объект.НаименованиеВалюты;
	Объект.СуммаПоДокументу = Объект.СписокНоменклатуры.Итог("Сумма");
	//30.03.24
...
#Область ДоговорСКонтрагентом
&НаКлиенте
Процедура КонтрагентПриИзменении(Элемент) 
	ЗагрузитьЗначениеНаСервере(Объект.Контрагент);
КонецПроцедуры

&НаСервере
Процедура ЗагрузитьЗначениеНаСервере(Владелец)
	Запрос = Новый Запрос;
	...
	|ГДЕ
	|	Договоры.ОсновнойДокумент = ИСТИНА
